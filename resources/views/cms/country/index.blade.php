...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div>
                <form action="{{ route('countries.index') }}" method="GET" class="ml-auto">
                    <div class="input-group">
                        <input type="text" name="name"
                            @if (request()->name) value={{ request()->name }} @endif class="form-control"
                ...
                <form action="{{ route('countries.index') }}" method="GET" class="ml-auto">
                    <div class="input-group">
                        <input type="text" name="code"
                            @if (request()->code) value={{ request()->code }} @endif class="form-control"
                            placeholder="Search about Code">
                        <div class="input-group-append">
                            <button type="submit" class="btn btn-primary">Search</button>
                        </div>
                    </div>
            ...
            <div style="color: red">
                <a
                href="{{ route('countries.index') }}" type="button" class="btn btn-danger">
            ...
            $("#add-country-form").on("submit", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $("#modal-alert").html(
                            '<div class="alert alert-success">تمت الإضافة بنجاح، سيتم الرجوع تلقائيًا...</div>'
                        );
                ...
                `);
                    ...
                    error: function() {
                        $("#modal-alert").html(
                            '<div class="alert alert-danger">فشل في الإضافة، تحقق من البيانات</div>'
                        );
                    }
                });
            });
